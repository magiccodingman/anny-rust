...
use super::*;
use anny_core::{AnnyF32, ModelOutputF32, PoseSessionF32, TensorF32};
...
pub unsafe extern "C" fn anny_model_to_f32(
    ...
    guard(|| {
        ...
        unsafe {
            *out = Box::into_raw(Box::new(AnnyModelF32 {
                model: Arc::new(model),
            }));
        }
        Ok(())
...
/// Create a reusable single-precision pose session; identical in meaning to `anny_session_new`,
/// over the f32 handle. The session keeps its own reference to the model.
/// # Safety
...
#[no_mangle]
pub unsafe extern "C" fn anny_session_f32_new(
    model: *const AnnyModelF32,
    parameters: *const c_char,
    out: *mut *mut AnnySessionF32,
) -> i32 {
    guard(|| {
        if out.is_null() {
            return Err("null f32 session handle".into());
        }
        unsafe {
            *out = ptr::null_mut();
        }
        ...
        // See `anny_session_new`: `owner` outlives the session and models are immutable.
        let model_ref: &'static AnnyF32 = unsafe { &*Arc::as_ptr(&owner) };
        let session = model_ref.pose_session(&p).map_err(|e| e.to_string())?;
        unsafe {
            *out = Box::into_raw(Box::new(AnnySessionF32 {
                _model: owner,
                session,
            }));
        }
...
/// Re-pose the f32 session; identical in meaning to `anny_session_update`.
/// # Safety
/// session is a live handle; pose_json is null or a valid NUL-terminated UTF-8 JSON pose document.
/// Views taken before this call are invalid afterwards.
#[no_mangle]
pub unsafe extern "C" fn anny_session_f32_update(
    session: *mut AnnySessionF32,
    pose_json: *const c_char,
) -> i32 {
    guard(|| {
        let s = unsafe { session.as_mut() }.ok_or("null f32 session")?;
...
/// Read one array of the f32 session's current result; see `anny_session_tensor`.
/// # Safety
/// session/name are valid live pointers; out points to writable view storage. Never reinterpret this
/// view with the double-precision layout.
#[no_mangle]
pub unsafe extern "C" fn anny_session_f32_tensor(
    session: *const AnnySessionF32,
    name: *const c_char,
    out: *mut AnnyTensorViewF32,
) -> i32 {
    guard(|| {
        ...
        let s = unsafe { session.as_ref() }.ok_or("null f32 session")?;
        let t = s
            .session
            .output()
            .get(unsafe { text(name)? })
...
/// Read the coefficients the f32 session was created with.
/// # Safety
/// session is a valid live pointer; out points to writable view storage.
#[no_mangle]
pub unsafe extern "C" fn anny_session_f32_coefficients(
    session: *const AnnySessionF32,
    out: *mut AnnyTensorViewF32,
) -> i32 {
    guard(|| {
        ...
        let s = unsafe { session.as_ref() }.ok_or("null f32 session")?;
        unsafe {
            *out = view32(s.session.coefficients());
        }
        Ok(())
    })
}
...
#[no_mangle]
pub unsafe extern "C" fn anny_session_f32_free(session: *mut AnnySessionF32) {
    if !session.is_null() {
        unsafe {
...
#[cfg(test)]
mod session_f32_tests {
    use super::*;
    #[test]
    fn f32_session_matches_evaluate_over_every_array() {
        let names: Vec<String> = fixture::tiny()
        ...
        let source = Box::into_raw(Box::new(AnnyModel {
            model: Arc::new(fixture::tiny()),
        }));
        let mut model = ptr::null_mut();
        assert_eq!(unsafe { anny_model_to_f32(source, &mut model) }, 0);
        let mut output = ptr::null_mut();
        assert_eq!(
            unsafe { anny_model_f32_evaluate(model, ptr::null(), &mut output) },
            0
        );
        let mut session = ptr::null_mut();
        assert_eq!(
            unsafe { anny_session_f32_new(model, ptr::null(), &mut session) },
            0
        );
        assert_eq!(unsafe { anny_session_f32_update(session, ptr::null()) }, 0);
        for name in &names {
            let cname = CString::new(name.as_str()).unwrap();
            let mut a = AnnyTensorViewF32::default();
            let mut b = AnnyTensorViewF32::default();
            assert_eq!(
                unsafe { anny_session_f32_tensor(session, cname.as_ptr(), &mut a) },
                0,
                "session tensor {name}"
            );
            assert_eq!(
                unsafe { anny_output_f32_tensor(output, cname.as_ptr(), &mut b) },
                0,
                "output tensor {name}"
            );
        ...
        let mut coefficients = AnnyTensorViewF32::default();
        assert_eq!(
            unsafe { anny_session_f32_coefficients(session, &mut coefficients) },
            0
        );
        assert!(coefficients.len > 0);
        // The f32 session also keeps its model alive on its own.
        unsafe {
            anny_model_f32_free(model);
        }
        assert_eq!(unsafe { anny_session_f32_update(session, ptr::null()) }, 0);
        assert_eq!(
            unsafe { anny_session_f32_update(ptr::null_mut(), ptr::null()) },
            1
        );
        assert_eq!(
            unsafe { anny_session_f32_tensor(session, ptr::null(), ptr::null_mut()) },
            1
        );
        unsafe {
            anny_session_f32_free(session);
            anny_session_f32_free(ptr::null_mut());
            anny_output_f32_free(output);
            anny_model_free(source);
        }
    }
}
