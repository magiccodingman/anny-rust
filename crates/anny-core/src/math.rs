...
pub fn vec3(x: &[f64]) -> Vec3 {
    Vec3::new(x[0], x[1], x[2])
}
pub fn mat3(x: &[f64]) -> Mat3 {
    Mat3::from_row_slice(x)
}
pub fn mat4(x: &[f64]) -> Mat4 {
    Mat4::from_row_slice(x)
}
pub fn write3(m: &Mat3, out: &mut [f64]) {
    for i in 0..3 {
        for j in 0..3 {
            out[i * 3 + j] = m[(i, j)];
        }
...
pub fn write4(m: &Mat4, out: &mut [f64]) {
    for i in 0..4 {
        for j in 0..4 {
            out[i * 4 + j] = m[(i, j)];
        }
...
pub fn legacy_roll_y(roll: f64) -> Mat3 {
    let r = roll as f32;
    let x = (r * r).sqrt() / 2.;
...
pub fn linear_interpolation(value: f64, anchors: &[f64], extrapolate: bool) -> Result<Vec<f64>> {
    ensure(
        value.is_finite()
            && anchors.len() >= 2
...
pub fn interpolate(value: f64, anchors: &[f64], values: &[f64], extrapolate: bool) -> Result<f64> {
...
pub fn dual_quaternion(h: &Mat4) -> ([f64; 4], [f64; 4]) {
...
pub fn dqs_point(
    v: &Vec3,
    influences: impl Iterator<Item = (f64, ([f64; 4], [f64; 4]))>,
) -> Result<Vec3> {
    let mut qr = [0.; 4];
    let mut qd = [0.; 4];
    ...
    for (w, (q, d)) in influences {
        ...
        let dot: f64 = (0..4).map(|i| q[i] * r[i] + d[i] * s[i]).sum();
    ...
    let norm = qr.iter().map(|v| v * v).sum::<f64>().sqrt();
